defmodule TeacExample.Accounts do
  import Ecto.Query
  import Ecto.Changeset

  alias TeacExample.Repo
  alias TeacExample.Accounts.{User, Identity}

  @pubsub TeacExample.PubSub
  ...
  def admin?(%User{} = user) do
    user.email in TeacExample.config([:admin_emails])
  end
  ...
  def update_public_settings(%User{} = user, attrs) do
    ...
    |> case do
      {:ok, new_user} ->
        # TeacExample.execute(__MODULE__, %Events.PublicSettingsChanged{user: new_user})
        {:ok, new_user}
